(function () {
  ...
  function wireHeroVideo() {
    ...
    // Backup for the native loop param (which can be unreliable): explicitly
    // restart when YouTube reports the video ended. This only listens to
    // postMessage events the embed already sends with enablejsapi=1 — it does
    // not load any extra script, so it can't be blocked by ad/privacy
    // blockers the way the full iframe_api library sometimes is.
